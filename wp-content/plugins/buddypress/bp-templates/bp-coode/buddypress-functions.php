<?php
/**
 * Functions of BuddyPress's Coode theme.
 *
 * @since 1.7.0
 *
 * @package BuddyPress
...
if ( !class_exists( 'BP_Coode' ) ) :

/**
 * Loads BuddyPress Coode Theme functionality.
 *
 ...
 * @subpackage BP_Theme_Compat
 */
class BP_Coode extends BP_Theme_Compat {

	/** Functions *************************************************************/

	/**
	 * The main BuddyPress (Coode) Loader.
	 *
	 * @since 1.7.0
	 *
	 */
	...
	protected function setup_globals() {
		$bp            = buddypress();
		$this->id      = 'coode';
		$this->name    = __( 'BuddyPress Coode', 'buddypress' );
		$this->version = bp_get_version();
		$this->dir     = trailingslashit( $bp->themes_dir . '/bp-coode' );
		$this->url     = trailingslashit( $bp->themes_url . '/bp-coode' );
	}
	...
	protected function setup_actions() {
		...
		if ( ! is_admin() || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
			...
			if ( bp_is_active( 'groups' ) ) {
				...
				add_action( 'bp_groups_directory_group_filter', 'bp_coode_theme_group_create_nav', 999           );
				add_action( 'bp_after_group_admin_content',     'bp_coode_groups_admin_screen_hidden_input'      );
				add_action( 'bp_before_group_admin_form',       'bp_coode_theme_group_manage_members_add_search' );
			}

			// Blog button.
			if ( bp_is_active( 'blogs' ) ) {
				add_action( 'bp_directory_blogs_actions',    'bp_blogs_visit_blog_button'           );
				add_action( 'bp_blogs_directory_blog_types', 'bp_coode_theme_blog_create_nav', 999 );
			}
		}
		...
		$actions = array(
			...
			// Directory filters.
			'blogs_filter'    => 'bp_coode_theme_object_template_loader',
			'forums_filter'   => 'bp_coode_theme_object_template_loader',
			'groups_filter'   => 'bp_coode_theme_object_template_loader',
			'members_filter'  => 'bp_coode_theme_object_template_loader',
			'messages_filter' => 'bp_coode_theme_messages_template_loader',
			'invite_filter'   => 'bp_coode_theme_invite_template_loader',
			'requests_filter' => 'bp_coode_theme_requests_template_loader',

			// Friends.
			'accept_friendship' => 'bp_coode_theme_ajax_accept_friendship',
			'addremove_friend'  => 'bp_coode_theme_ajax_addremove_friend',
			'reject_friendship' => 'bp_coode_theme_ajax_reject_friendship',

			// Activity.
			'activity_get_older_updates'  => 'bp_coode_theme_activity_template_loader',
			'activity_mark_fav'           => 'bp_coode_theme_mark_activity_favorite',
			'activity_mark_unfav'         => 'bp_coode_theme_unmark_activity_favorite',
			'activity_widget_filter'      => 'bp_coode_theme_activity_template_loader',
			'delete_activity'             => 'bp_coode_theme_delete_activity',
			'delete_activity_comment'     => 'bp_coode_theme_delete_activity_comment',
			'get_single_activity_content' => 'bp_coode_theme_get_single_activity_content',
			'new_activity_comment'        => 'bp_coode_theme_new_activity_comment',
			'post_update'                 => 'bp_coode_theme_post_update',
			'bp_spam_activity'            => 'bp_coode_theme_spam_activity',
			'bp_spam_activity_comment'    => 'bp_coode_theme_spam_activity',

			// Groups.
			'groups_invite_user' => 'bp_coode_theme_ajax_invite_user',
			'joinleave_group'    => 'bp_coode_theme_ajax_joinleave_group',

			// Messages.
			'messages_autocomplete_results' => 'bp_coode_theme_ajax_messages_autocomplete_results',
			'messages_close_notice'         => 'bp_coode_theme_ajax_close_notice',
			'messages_delete'               => 'bp_coode_theme_ajax_messages_delete',
			'messages_markread'             => 'bp_coode_theme_ajax_message_markread',
			'messages_markunread'           => 'bp_coode_theme_ajax_message_markunread',
			'messages_send_reply'           => 'bp_coode_theme_ajax_messages_send_reply',
		);

		// Conditional actions.
		if ( bp_is_active( 'messages', 'star' ) ) {
			$actions['messages_star'] = 'bp_coode_theme_ajax_messages_star_handler';
		}
		...
		add_filter( 'bp_ajax_querystring', 'bp_coode_theme_ajax_querystring', 10, 2 );
		 ...
		 * @since 1.7.0
		 *
		 * @param BP_Coode $this Current BP_Coode instance.
		 */
	...
	public function enqueue_scripts() {
		 ...
		 */
		$store_filter_settings = apply_filters( 'bp_coode_store_filter_settings', is_user_logged_in() );
		...
		if ( bp_is_register_page() || ( function_exists( 'bp_is_user_settings_general' ) && bp_is_user_settings_general() ) ) {
			...
			$asset = $this->locate_asset_in_stack( "password-verify{$min}.js", 'js', 'bp-coode-password-verify' );
	...
	public function theme_compat_page_templates( $templates = array() ) {
		 ...
		 */
		if ( true === (bool) apply_filters( 'bp_coode_theme_compat_page_templates_directory_only', ! bp_is_directory() ) ) {
			return $templates;
		}
 ...
 *
 * The bp-coode puts the Create a Group button into the page title, to mimic
 ...
 * @return string
 */
function bp_coode_theme_group_create_button( $title ) {
 ...
 *
 * The bp-coode puts the Create a Group nav at the last position of
 * the Groups directory navigation.
 *
 * @since 2.2.0
 *
 */
function bp_coode_theme_group_create_nav() {
 ...
 */
function bp_coode_groups_admin_screen_hidden_input() {
	?>
 ...
 *
 * The bp-coode puts the Create a Site button into the page title, to mimic
 * the behavior of bp-default.
 *
 * @since 2.0.0
 * @todo Deprecate
 ...
 * @return string
 */
function bp_coode_theme_blog_create_button( $title ) {
 ...
 *
 * The bp-coode puts the Create a Site nav at the last position of
 * the Sites directory navigation.
 *
 * @since 2.2.0
 *
 */
function bp_coode_theme_blog_create_nav() {
 ...
 */
function bp_coode_theme_ajax_querystring( $query_string, $object ) {
	if ( empty( $object ) )
		return '';
	 ...
	 */
	return apply_filters( 'bp_coode_theme_ajax_querystring', $query_string, $object, $object_filter, $object_scope, $object_page, $object_search_terms, $object_extras );
}
 ...
 */
function bp_coode_theme_object_template_loader() {
	...
	$template_path = apply_filters( 'bp_coode_object_template_path', $template_path );
 ...
 */
function bp_coode_theme_messages_template_loader() {
 ...
 * @since 2.0.0
 */
function bp_coode_theme_invite_template_loader() {
 ...
 * @since 2.0.0
 */
function bp_coode_theme_requests_template_loader() {
 ...
 */
function bp_coode_theme_activity_template_loader() {
	if ( ! bp_is_post_request() ) {
		return;
	}
	 ...
	 */
	$result['feed_url'] = apply_filters( 'bp_coode_theme_activity_feed_url', $feed_url, $scope );
	ob_end_clean();
 ...
 * @return string|null HTML
 */
function bp_coode_theme_post_update() {
	$bp = buddypress();
 ...
 * @return string|null HTML
 */
function bp_coode_theme_new_activity_comment() {
 ...
 */
function bp_coode_theme_delete_activity() {
	if ( ! bp_is_post_request() ) {
		return;
	}
 ...
 * @return mixed String on error, void on success.
 */
function bp_coode_theme_delete_activity_comment() {
	if ( ! bp_is_post_request() ) {
		return;
	}
 ...
 * @return mixed String on error, void on success.
 */
function bp_coode_theme_spam_activity() {
	$bp = buddypress();
 ...
 * @return string|null HTML
 */
function bp_coode_theme_mark_activity_favorite() {
 ...
 * @return string|null HTML
 */
function bp_coode_theme_unmark_activity_favorite() {
	if ( ! bp_is_post_request() ) {
		return;
	}
 ...
 * @return string|null HTML
 */
function bp_coode_theme_get_single_activity_content() {
	if ( ! bp_is_post_request() ) {
		return;
	}
	 ...
	 */
	do_action_ref_array( 'bp_coode_theme_get_single_activity_content', array( &$activity ) );
 ...
 */
function bp_coode_theme_ajax_invite_user() {
	if ( ! bp_is_post_request() ) {
		return;
	}
 ...
 * @return string|null HTML
 */
function bp_coode_theme_ajax_addremove_friend() {
	if ( ! bp_is_post_request() ) {
		return;
	}
 ...
 * @return mixed String on error, void on success.
 */
function bp_coode_theme_ajax_accept_friendship() {
	if ( ! bp_is_post_request() ) {
		return;
	}
 ...
 * @return mixed String on error, void on success.
 */
function bp_coode_theme_ajax_reject_friendship() {
	if ( ! bp_is_post_request() ) {
		return;
	}
 ...
 * @return string|null HTML
 */
function bp_coode_theme_ajax_joinleave_group() {
	if ( ! bp_is_post_request() ) {
		return;
	}
 ...
 * @return mixed String on error, void on success.
 */
function bp_coode_theme_ajax_close_notice() {
	if ( ! bp_is_post_request() ) {
		return;
	}
 ...
 * @return string|null HTML
 */
function bp_coode_theme_ajax_messages_send_reply() {
	if ( ! bp_is_post_request() ) {
		return;
	}
 ...
 * @return mixed String on error, void on success.
 */
function bp_coode_theme_ajax_message_markunread() {
	die( '-1' );
}
 ...
 * @return mixed String on error, void on success.
 */
function bp_coode_theme_ajax_message_markread() {
	die( '-1' );
}
 ...
 * @return string|null HTML
 */
function bp_coode_theme_ajax_messages_delete() {
	die( '-1' );
}
 ...
 * @since 1.2.0
 */
function bp_coode_theme_ajax_messages_autocomplete_results() {
 ...
 * @since 2.3.0
 */
function bp_coode_theme_ajax_messages_star_handler() {
...
/**
 * BP Coode's callback for the cover image feature.
 *
 * @since  2.4.0
 *
 ...
 */
function bp_coode_theme_cover_image( $params = array() ) {
	if ( empty( $params ) ) {
		return;
	}
 ...
 */
function bp_coode_theme_group_manage_members_add_search() {
